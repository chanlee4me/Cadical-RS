...
void Internal::bump_variable_score (int lit, int glue) {
  assert (opts.bump);
  int idx = vidx (lit);
...
void Internal::bump_variable (int lit, int glue) {
  if (use_scores ())
    bump_variable_score (lit, glue);
  else
    bump_queue (lit);
}
...
void Internal::bump_variables (int glue) {
